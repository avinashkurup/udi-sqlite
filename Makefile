...
# # Add http extension, GO compilation target and related vars.
# # Variables

# HTTP_GO_SRC_DIR = sqlite-http
# #GO_FILES= ./cookies.go ./settings.go ./do.go ./shared.go ./meta.go ./headers.go
# #prefix = $(HTTP_GO_SRC_DIR)/dist
# prefix = dist

# # HTTP_EXT_COMMIT=$(shell cd $(HTTP_GO_SRC_DIR) && git rev-parse HEAD)
# # HTTP_EXT_VERSION=$(shell cd $(HTTP_GO_SRC_DIR) && cat VERSION)
# # HTTP_DATE=$(shell date +'%FT%TZ%z')
# # GO_BUILD_LDFLAGS=-ldflags '-X main.Version=v$(VERSION) -X main.Commit=$(COMMIT) -X main.Date=$(DATE)'
# # GO_BUILD_CGO_CFLAGS=CGO_ENABLED=1 CGO_CFLAGS="-DUSE_LIBSQLITE3"

# HTTP_GO_BUILD_CGO_CFLAGS=CGO_CFLAGS=-DSQLITE3_INIT_FN=sqlite3_http_init
...
# # Directory and target setups
# TARGET_STATIC_LIB=$(prefix)/libsqlite_http0.$(LOADABLE_EXTENSION)

# # Targets
# prepare_http_dist:
# 	mkdir -p $(HTTP_GO_SRC_DIR)/$(prefix)

# http_static_lib: $(TARGET_STATIC_LIB)

# $(TARGET_STATIC_LIB):  $(shell find $(HTTP_GO_SRC_DIR) -type f -name '*.go')
# 	cd $(HTTP_GO_SRC_DIR) && $(HTTP_GO_BUILD_CGO_CFLAGS) go build -buildmode=c-archive \
# 	$(GO_BUILD_LDFLAGS) \
# 	-o $@ .

# # $(TARGET_STATIC_LIB):  $(GO_FILES)
# # 	cd $(HTTP_GO_SRC_DIR_DIR) && $(GO_BUILD_CGO_CFLAGS) CGO_ENABLED=1 go build -buildmode=c-archive \
# # 	$(GO_BUILD_LDFLAGS) \
# # 	-o $@ .

# sqlite_http_clean_libs:
# 	rm -rf $(HTTP_GO_SRC_DIR)/$(prefix)/*

# # format:
# # 	gofmt -s -w .


# BEGIN ChatGPT suggested http and html extension Make targets. For compiling Go.
...
# html
# linux LDFLAGS: -Wl,--unresolved-symbols=ignore-in-object-files

# http
# linux LDFLAGS: -Wl,--unresolved-symbols=ignore-in-object-files
prefix=dist

TARGET_STATIC_LIB_HTML=$(prefix)/html0.a
TARGET_STATIC_LIB_HTTP=$(prefix)/http0.a

GO_FILES_HTML= $(shell find . -type f -name '*.go')
GO_FILES_HTTP= ./cookies.go ./settings.go ./do.go ./shared.go ./meta.go ./headers.go

HTML_SRC_DIR = sqlite-html
...
html-prepare-dist:
	mkdir -p $(HTML_SRC_DIR)/$(prefix)

http-prepare-dist:
	mkdir -p $(HTTP_SRC_DIR)/$(prefix)

$(TARGET_STATIC_LIB_HTML): $(GO_FILES_HTML)
	cd $(HTML_SRC_DIR) && $(GO_BUILD_CGO_CFLAGS) go build \
	-buildmode=c-archive -o $@ \
	$(GO_BUILD_LDFLAGS) \
	.

$(TARGET_STATIC_LIB_HTTP): $(GO_FILES_HTTP)
	cd $(HTTP_SRC_DIR) && $(GO_BUILD_CGO_CFLAGS) go build \
	-buildmode=c-archive -o $@ \
	$(GO_BUILD_LDFLAGS) \
	.

all: $(TARGET_STATIC_LIB_HTML) $(TARGET_STATIC_LIB_HTTP)

clean_html:
	rm -rf $(HTML_SRC_DIR)/$(prefix)/*

clean_http:
	rm -rf $(HTTP_SRC_DIR)/$(prefix)/*

.PHONY: all clean


# END ChatGPT suggested http extension Make targets.
